...
- **Initial local Skill access, no native MCP standard yet**: The current repository can load existing Claude/Codex-style `SKILL.md` packages from user skill directories, but it still has no first-class MCP server/client loader and no mature permissioned Skill lifecycle.
...
- The tool list is not actually a clear "9 atomic tools" set. `assets/tools_schema*.json` currently exposes 11 function tools: `code_run`, `file_read`, `file_patch`, `file_write`, `web_scan`, `web_execute_js`, `update_working_checkpoint`, `load_skill`, `ask_user`, `codex_lesson_update`, and `start_long_term_update`. Several of these are memory, Skill, or distillation helpers, not atomic environment tools.
...
- There is now an initial local Agent Skill loader: `load_skill` can discover and load Claude/Codex-style `SKILL.md` packages from `~/.claude/skills` and `~/.codex/skills`. Discovery builds a cached metadata index and deduplicates repeated names by search-root order (`~/.claude/skills` before `~/.codex/skills` by default); full Skill content is loaded only when `load_skill` is called. This is still not a mature external Skill runtime because permissioning, versioning, lifecycle management, and MCP-provided skills remain incomplete.
...
3️⃣ **Minimal Toolset**
> _GenericAgent currently uses a static JSON tool schema. The README used to call this "9 atomic tools", but the repository now exposes 11 function tools and mixes environment tools with Skill/memory/distillation helpers._
...
| `load_skill`                | Load local Claude/Codex-style SKILL.md |
...
> _Can load local Claude/Codex-style skills and can ask the model to write/run scripts, but this is not yet a fully governed external capability system._

Via `load_skill`, GenericAgent can reuse existing `SKILL.md` packages from the user's Claude/Codex skill directories. It indexes only Skill metadata in the prompt, caches repeated discovery while files are unchanged, deduplicates same-name Skills by root priority, and loads the full body on demand. Via `code_run`, it can also dynamically install Python packages, write new scripts, call external APIs, or control hardware at runtime. The latter should still be treated as ad-hoc local code execution, not as a safe replacement for MCP or a mature Agent Skill framework.
...
- **已有初始本地 Skill 接入，但尚无原生 MCP 标准**: 当前仓库可以加载用户目录里的 Claude/Codex 风格 `SKILL.md` 包，但还没有一等 MCP server/client 加载路径，也没有成熟的权限化 Skill 生命周期管理。
...
- 工具数量并不是清晰的“9 个原子工具”。`assets/tools_schema*.json` 当前实际暴露 11 个函数工具：`code_run`、`file_read`、`file_patch`、`file_write`、`web_scan`、`web_execute_js`、`update_working_checkpoint`、`load_skill`、`ask_user`、`codex_lesson_update`、`start_long_term_update`。其中不少是 Skill、记忆或蒸馏辅助，不是同一层级的环境原子工具。
- 当前没有原生 MCP 接入路径。ACP bridge 明确声明 MCP HTTP/SSE 能力为 false，工具分发也只是调用本地 `do_<tool>` 方法。
- 当前已有初始本地 Agent Skill 加载器：`load_skill` 可以发现并加载 `~/.claude/skills` 与 `~/.codex/skills` 中的 Claude/Codex 风格 `SKILL.md` 包。发现阶段会构建带缓存的元数据索引，并按搜索目录顺序对重名 Skill 去重（默认 `~/.claude/skills` 优先于 `~/.codex/skills`）；完整 Skill 内容只在调用 `load_skill` 时加载。但这仍不是成熟外部 Skill 运行时，因为权限、版本、生命周期管理，以及 MCP 提供的 skill 接入还不完整。
...
3️⃣ **最小工具集**
> GenericAgent 当前使用静态 JSON 工具 schema。README 过去称其为“9 个原子工具”，但仓库现状实际暴露 11 个函数工具，并且混合了环境控制工具、Skill 工具、记忆工具和蒸馏工具。
...
| `load_skill`                | 加载本地 Claude/Codex 风格 SKILL.md |
...
4️⃣ **能力扩展机制**
> 可以加载本地 Claude/Codex 风格 skills，也可以让模型编写并运行脚本，但这还不是完整有治理的外部能力系统。
>
通过 `load_skill`，GenericAgent 可以复用用户 Claude/Codex skill 目录中的既有 `SKILL.md` 包。它只把 Skill 元数据索引注入 prompt，在文件未变化时复用发现缓存，对同名 Skill 按目录优先级去重，并在需要时才加载完整正文。通过 `code_run`，GenericAgent 仍可在运行时动态安装 Python 包、编写新脚本、调用外部 API 或控制硬件；后者应被视为临时本地代码执行，而不是 MCP 或成熟 Agent Skill 框架的安全替代品。
